...
public interface MedicineRepository extends JpaRepository<Medicine, String> {
    ...
    @Query("SELECT m FROM Medicine m WHERE " +
            ...
            "((:color1 IS NULL OR m.colorClass1 LIKE %:color1%) OR (:color2 IS NULL OR m.colorClass2 LIKE %:color2%) " +
            "OR (:color1 IS NULL OR m.colorClass2 LIKE %:color1%) OR (:color2 IS NULL OR m.colorClass1 LIKE %:color2%)) AND " +
